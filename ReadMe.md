...
- **UI Framework:** WinUI3 (.NET 9) / Windows App SDK
- **Language:** C#

### AI & Inference Environment (WSL2)

- **AI Model:** Meta SAM2 (Segment Anything Model 2)
- **Environment:** Ubuntu (Windows Subsystem for Linux 2) / Python venv
- **Hardware Acceleration:** PyTorch with Multi-GPU selection support

### Bridge & IPC (Inter-Process Communication)

- **Execution:** Subprocess spawning (`ubuntu run ...`)
- **Data Exchange:** File-system based IPC (Read/Write via shared Windows directory)
...
graph TD
    ...
    subgraph SharedFS [📁 Shared Directory IPC]
        Image[Original Slide Image]
        CSV[Coordinates & Mask CSV]
    end

    %% WSL Backend
    subgraph WSLGuest [🐧 WSL 2: AI Backend]
        SAM2[Meta SAM2 Model / Multi-GPU]
        PythonEnv[Python Subprocess]
        
        PythonEnv -->|1. Load Image| Image
        PythonEnv -->|2. Batch Inference| SAM2
        SAM2 -->|3. Extract All Masks| CSV
    end

    %% Windows Frontend
    subgraph WindowsHost [🖥️ Windows Host: WinUI 3]
        UI[Interactive UI & Viewer]
        SubProcess[Trigger `ubuntu run`]
        InMemory[Local Memory / State]

        %% Phase 1: Init Flow
        UI -->|0. Select File| SubProcess
        SubProcess -->|Execute Once| PythonEnv
        
        %% Phase 2: Interaction Flow
        CSV -->|4. Parse to RAM| InMemory
        UI <-->|5. Point-click, Color Analysis| InMemory
    end
```
...
|---|---|---|---|
| ML inference | Python/SAM2 via WSL subprocess | ONNX Runtime + DirectML | Native GPU, no WSL, no subprocess |
| IPC (if keep Python) | `ubuntu run {cli_string}` | FastAPI + `HttpCLient.PostAsJsonAsync` | Typed JSON, async, no CLI injection |
| Settings storage | `Local Settings` string keys | `LocalSettings` + typed constants class | Same API, add compile-time safety |
| Preset storage | `.txt` with `Split(": ")` parser | `System.Text.Json` + typed POCO | Robust, no custom parser |
| Mask storage | CSV (0/1 text) | NPY or PNG | 30-50x smaller, standard tooling |
| WSL detection | WMI `Win32_OptionalFeature` | `wsl --status` subprocess | Simpler, locale-independent |
| GPU enumeration | NvAPIWrapper + WMI redundancy | NvAPIWrapper only (or DirectML device enum) | One path |
...
> #### Select Points<br>
> - Fixed an issue where points were not displayed properly in Manual Segmentation mode<br>
> #### History<br>
...
> #### Segmentation<br>
> - Manual Segmentation is now available.<br>
